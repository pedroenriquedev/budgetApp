...
var budgetController = (function () {
  ...
  return {
    ...
    calculateCategoriesPercentage: (totalsArr) => {
      let percArr;
      percArr = totalsArr.map((cur) =>
        ((cur / data.totals.exp) * 100).toFixed(2)
      );
      return percArr;
    },
...
var UIController = (function () {
  ...
  return {
    ...
    getInput: () => {
      let isInvalid = inputValueValidate(
    ...
    addListItem: (obj, type) => {
      ...
      if (type === "inc") {
        ...
        html = `<div class="item" id="inc-${
          obj.id
        }"><div class="left"><div class="item__description" id="description-inc-${
          obj.id
        }">${
          obj.description
        }</div></div><div class="right"><div class="item__value" id="value-inc-${
          obj.id
        }">${formatNumber(
          obj.value,
          "inc"
        )}</div><div class="item__edit"><button class="item__edit--btn"><i class="far fa-edit " id="edit-inc-${
          obj.id
        }"></i></button></div><div class="item__delete"><button class="item__delete--btn"><i class="fas fa-times" id="delete-inc-${
          obj.id
        }"></i></button></div></div><div class="invalidInput" id="invalid-${
          obj.id
        }">Invalid Input</div></div>
        ...
        html = `<div class="item clearfix" id="exp-${obj.id}">
                <div class="left">
                <div class="item__description" id="description-exp-${obj.id}">${
          obj.description
        }</div>
                ...
                <div class="right">
                <div class="item__value" id="value-exp-${
                  obj.id
                }">${formatNumber(obj.value, "exp")}</div>
                ...
                <button class="item__delete--btn"><i class="fas fa-times" id="delete-exp-${
                  obj.id
                }"></i></button>
                </div>
                </div>
                <div class="invalidInput" id="invalid-${
                  obj.id
                }">Invalid Input</div>
                </div>`;
      }
    ...
    displayBudget: (budgetData) => {
      ...
      document.querySelector(DOMstrings.expensesLabel).textContent =
        formatNumber(budgetData.totalExp, "exp");
    ...
    displayMonth: () => {
      ...
      document.querySelector(DOMstrings.dateLabelMonth).textContent =
        formattedDateMonth;

      document.querySelector(DOMstrings.dateLabelYear).textContent =
        " " + formattedDateYear;
    },
    ...
    displayProgressBars: (categories, categoriesPerc, categoriesTotals) => {
      ...
      for (let i = 0; i < categoriesLength; i++) {
        html = `<div class="budget__categoryTotals--category">${
          categories[i]
        }</div><div class="budget__categoryTotals--bar"><span style="width: ${
          categoriesPerc[i]
        }%"></span></div><div class="budget__categoryTotals--value">$ ${categoriesTotals[
          i
        ].toFixed(2)}</div>`;
        document
          .querySelector(DOMstrings.categoriesChart)
...
var controller = (function (budgetCtrl, UICtrl) {
  ...
  var setupEventListeners = function () {
    document
      ...
      .addEventListener("click", crtlAddItem);
    ...
    document
      .querySelector(DOM.deleteAllButton)
  ...
  var ctrlEditItem = (e) => {
    ...
    if (e.target.id.includes("edit-")) {
      ...
      descriptionBox = document.getElementById(
      ...
      if (e.target.classList.contains("fa-edit")) {
        ...
        if (newDesc.length > 25 || isValueValid === false) {
          ...
          budgetCtrl.changeObjectProperty(
            obj.id,
            obj.type,
            "description",
            newDesc
